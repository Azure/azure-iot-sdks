...
def get_iothub_opt(argv, connection_string, protocol=IoTHubTransportProvider.AMQP):
    if len(argv) > 0:
        try:
        ...
        for opt, arg in opts:
            if opt == '-h':
                ...
                if (protocol_string == "amqp"):
                    ...
                    raise OptionError("Error: unknown protocol %s" % protocol_string)
            elif opt in ("-c", "--connectionstring"):
                connection_string = arg

    if (connection_string.find("HostName") < 0):
        raise OptionError("Error: Hostname not found, not a valid connection string")
