    ...
    sealed class MqttIotHubAdapter : ChannelHandlerAdapter
    {
        ...
        public override async Task WriteAsync(IChannelHandlerContext context, object data)
        {
            try
            {
                ...
                {
                    await this.SubscribeAsync(context).ConfigureAwait(false);
                    return;
                }
        ...
        async Task SubscribeAsync(IChannelHandlerContext context)
        {
            ...
            await this.subscribeCompletion.Task.ConfigureAwait(false);
        }
        ...
        async Task SendMessageAsync(IChannelHandlerContext context, Message message)
        {
            ...
            PublishPacket packet = await Util.ComposePublishPacketAsync(context, message, this.mqttTransportSettings.PublishToServerQoS, topicName).ConfigureAwait(false);
