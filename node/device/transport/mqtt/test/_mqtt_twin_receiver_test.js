...
describe('MqttTwinReceiver', function () {
  ...
  beforeEach(function(done) {
    ...
    receiverConfig = {'client' : provider};
    receiver = new MqttTwinReceiver(receiverConfig);
    done();
  });
    
  describe('#constructor', function () {
    
    /* Tests_SRS_NODE_DEVICE_MQTT_TWIN_RECEIVER_18_001: [** The `MqttTwinReceiver` constructor shall accept a `config` object **]** */
    it ('accepts a config object', function() {
      assert.equal(receiver._config, receiverConfig);
    });

    /* Tests_SRS_NODE_DEVICE_MQTT_TWIN_RECEIVER_18_002: [** The `MqttTwinReceiver` constructor shall throw `ReferenceError` if the `config` object is falsy **]** */
  ...
  describe('response event', function() {
    ...
    it ('asynchronously subscribes when  a listener is added', function(done) {
      validateSubscription('response', '$iothub/twin/res/#', done);
    });
    ...
    it ('unsubscribes when there are no more listeners', function(done) {
      validateUnsubscription('response', '$iothub/twin/res/#', done);
    });
  ...
  describe('post event', function() {
    ...
    it ('asynchronously subscribes when  a listener is added', function(done) {
      validateSubscription('post', '$iothub/twin/PATCH/properties/desired/#', done);
    });
    ...
    it ('unsubscribes when there are no more listeners', function(done) {
      validateUnsubscription('post', '$iothub/twin/PATCH/properties/desired/#', done);
    });
    ...
    it ('emits a response for every message', function(done) {
      validateEventFires('post', '$iothub/twin/PATCH/properties/desired/#', done);
    });
    ...
    it ('passes the body as a parameter of the event', function(done) {
      receiver.on('post', function(data) {
