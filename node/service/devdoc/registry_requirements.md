...
## Overview
`Registry` represents an IoT hub’s device identity service, known as the Registry.  A consumer can add, remove, update, or read device metadata from the Registry through the transport supplied by the caller (e.g., azure-iothub.Http—currently the Registry service only supports HTTP).
## Example usage
