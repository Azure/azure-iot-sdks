...
HTTP_HANDLE HTTPAPI_CreateConnection(const char* hostName)
{
	...
	if (hostName)
	{
		...
		else if (!handle->con.load_certificate((const unsigned char*)certificates, strlen(certificates)))
		{
			LogError("HTTPAPI_CreateConnection::Could not load certificates\r\n");
			delete handle;
			handle = NULL;
		}
		else if (!handle->con.connect(hostName, 443))
		{
